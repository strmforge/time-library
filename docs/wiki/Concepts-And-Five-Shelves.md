...
| Preference | Preference, intent, corrections, and stable understanding | User likes concise answers; a project must not change network access; a prior interpretation was wrong |
| Work experience | Work experience, methods, validation paths, and adoption state | How to validate a Windows install; how to split a module safely; known repair sequence |
...
This is why the work-experience shelf is not a skill marketplace. It is the work-experience shelf
...
Time Library的核心能力可以分成两类：**记忆** 和 **经验**。
...
| `xingce` | 工作经验、方法、验证路径、采纳状态 | Windows 安装怎么验；模块怎么按公共规则边界拆；已知修复顺序 |
| `toolbook` | 工具使用知识和平台事实 | 某工具如何存数据；安全能力检查怎么跑 |
| `errata` | 纠错、冲突、信任修复 | 错误记忆、来源冲突、被拒绝或废弃的经验 |

这五层就是 AI 可读知识沉积的位置，不需要再造一个类似 Obsidian 的第六层。Obsidian/Markdown 的启发在于：纯文本、可链接、可 diff、AI 好读；不是说Time Library要变成 Obsidian 集成。
...
经验层不是技能市场。它更像经验的生命周期：候选、证据、复核、采纳、复用、纠错或回滚。每一步都应该带来源和回执。
