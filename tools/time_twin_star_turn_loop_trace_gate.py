#!/usr/bin/env python3
"""Read-only gate for external Time Twin Star real turn traces."""
...
def _print_text(payload: dict[str, Any]) -> None:
    print("# Time Twin Star Turn-Loop Trace Gate")
    print()
    print(f"- contract: `{payload.get('contract', '')}`")
    print(f"- gate status: `{payload.get('gate_status', '')}`")
    print(f"- behavior status: `{payload.get('turn_loop_behavior_status', '')}`")
    print(f"- sufficient: `{payload.get('trace_sufficient_for_behavior_proven', False)}`")
    print()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Classify an externally supplied real turn trace.")
    ...
    if args.definition:
        payload = time_twin_star_turn_loop_trace_gate_definition()
    else:
        payload = time_twin_star_turn_loop_trace_gate_from_observation(_load_trace(args.trace_json))
