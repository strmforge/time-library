...
忆凡尘让 Claude、Codex、OpenClaw、Hermes 和其他本机 AI 工具保留可回源的工作上下文：找回旧对话和偏好，复用做成过的修复办法，并且能回到原始记录核对。

当前候选版本：**2026.6.15**

2026.6.15 是当前本地候选版本，尚未发布。

当前轮本机 macOS 与两台 Windows 主机候选验证已完成；尚未 push、tag，也尚未发布 GitHub Release。

## 功能

- **跨工具本机上下文**：Claude Desktop、Claude Code CLI、Codex、OpenClaw、Hermes、Cursor 类工具和常见开源 agent 可以共用同一个本机记录底座。
- **自动保留本机记录**：把有用的本机 AI 对话和工具线索留在自己的电脑上，按设备和来源工具归档。
- **带来源的召回**：问旧决定、偏好、修复办法或项目边界时，默认返回精简来源线索、命中理由和可选的有界原文摘录。
- **可复用工作路径**：把排障顺序、复核步骤、项目规矩、踩坑记录和验收办法沉淀下来，让下一个 AI 窗口接着用。
...
- 公开文档、平台目录、watchlist、诊断和测试不再把某个本地中转工具写成公开依赖或平台入口。
- 已有个人中转痕迹仍通过中性的 `local_relay` 兼容识别，但不把它宣传成必要基础设施。
- 记录诊断继续使用遗失源 / 遗失 raw 说法。
- release gate 会扫描公开面和仓库文本，阻止被移除的中转工具名和不合规记录诊断回流。
- 安装器、gateway health、active memory routing、preflight metadata、本地控制台版本文本和随包 Zhiyi skill 都统一报告 2026.6.15。
- 经验验证回执、采纳门禁引用回执、采纳包预览和经验链路总览作为可回源 dry-run 治理链路进入本地候选。
- 当前候选版已完成本轮本机 macOS 与两台 Windows 主机验证：本地全量测试、working-tree release gate、Windows 原生 smoke 和记录链路巡检均已通过，未发现遗失源或遗失 raw。

当前候选说明见 [RELEASE_NOTES_2026.6.15.md](RELEASE_NOTES_2026.6.15.md)，完整历史更新见 [UPDATE_HISTORY.md](UPDATE_HISTORY.md)，更底层变更见 [CHANGELOG.md](CHANGELOG.md)。
