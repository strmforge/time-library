...
忆凡尘是一个本机 AI 记忆与经验系统。它把本机 AI 工具用过的有用线索留在你的电脑上，让下一次对话不用从零开始，并且每条记忆和经验都尽量能回到原始记录。

当前候选版本：**2026.6.14**

2026.6.14 是当前本地候选版本，尚未发布。

当前轮验证已在本机 macOS、Windows191、Windows123 通过；两台 Windows 都报告 `raw_sync=raw_current`。
...
- **经验可以跨窗口继续用**：偏好留在记忆里，做成过的办法沉淀成可复用经验。
...
- 安装器、gateway health、active memory routing、preflight metadata、本地控制台版本文本和随包 Zhiyi skill 都统一报告 2026.6.14。
- 当前候选版的多机验证仍需以本轮真实运行结果为准。

当前候选说明见 [RELEASE_NOTES_2026.6.14.md](RELEASE_NOTES_2026.6.14.md)，完整历史更新见 [UPDATE_HISTORY.md](UPDATE_HISTORY.md)，更底层变更见 [CHANGELOG.md](CHANGELOG.md)。
