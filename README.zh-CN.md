...
- Windows 后台常驻更稳，托盘图标可以直接打开本地页面、查看状态和补扫漏掉的记录。
- macOS 也有菜单栏入口，不用记 `9850` 端口。
- 本机聊天记录更接近实时同步：watcher 会持续运行，重启或修复安装后可以继续补上漏掉的记录。
- 知意只保留一个可见模型设置；如果你配置了模型，它也能帮忙识别陌生本机 AI 工具。默认扫描仍然只看本机元数据，不读聊天正文。
- Codex、Claude Desktop、Claude Code CLI、OpenClaw、Hermes 和常见开源 agent 会按各自入口分开看，召回时保留来源和边界。
- 默认召回仍然是 active 分层：当前窗口/session 优先，然后同项目/同工作区、同工作流/同任务、稳定偏好/工具事实；raw-pool/global 只在明确要求更宽视图时使用。
- Windows 继续默认走原生安装；WSL 只保留给开发和高级测试。

当前发布说明见 [RELEASE_NOTES_2026.6.6.md](RELEASE_NOTES_2026.6.6.md)，完整历史更新见 [UPDATE_HISTORY.md](UPDATE_HISTORY.md)，更底层变更见 [CHANGELOG.md](CHANGELOG.md)。
