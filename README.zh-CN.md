...
2026.6.20 是当前已发布版本。可以下载 release zip，或使用 GitHub Release
里的版本化安装脚本。
...
跑分有意义，但不能和日常使用混在一起。忆凡尘把日常召回、定向回归、离线 benchmark 分成不同入口，避免评分任务拖住正在使用的工作机。

第一层公开诊断是完全免费的证据检索，用公开记忆 benchmark 检查系统能不能把原始证据找回来：
...
| 数据集 | exact source recall | bundled source recall |
|---|---:|---:|
| LoCoMo locomo10 | 66.5/100 | 82.3/100 |
| LongMemEval oracle | 82.6/100 | 91.2/100 |

这套命令不调用裁判模型、不写记忆，也不声明 LoCoMo 或 LongMemEval 官方榜单分。

答案级诊断是另一条线。当前内部 LongMemEval oracle 判分暴露的问题很清楚：证据检索已经高于答案合成。最新 500 题内部诊断是 **39.4/100** 官方近似二分类 accuracy，或 **43.7/100** 内部半分制 answer acceptance。它只能用于错例复盘和质量对比，不能当官方榜单分；官方 LongMemEval 仍要走官方 evaluator 和对应模型环境。

具体命令、样本分层、Codex 内部强判官、资源账和 R730XD 离线压力测试见 [benchmarks/README.md](benchmarks/README.md)。
...
当前已发布版本是 **2026.6.20**。这一版重点修正本机 AI 工具接入边界、低资源默认、开工前上下文检查、记录医生、可回源召回，以及证据绑定模型诊断。

发布说明见 [RELEASE_NOTES_2026.6.20.md](RELEASE_NOTES_2026.6.20.md)，完整历史更新见 [UPDATE_HISTORY.md](UPDATE_HISTORY.md)，更底层变更见 [CHANGELOG.md](CHANGELOG.md)。
