...
2026.7.7.2 是当前已发布版本。可以下载 release zip，或使用 GitHub Release
里的版本化安装脚本。
...
当前已发布版本是 **2026.7.7.2**。这一版是维护更新，优化本地控制台交互、设置页加载、模型偏好控件和发布包一致性。旧 `memcore-cloud` 目录仍作为迁移源和卸载兜底保留，避免老用户数据丢失；本机 AI 工具接入边界、开工前上下文检查、记录医生、可回源召回，以及证据绑定回答路径继续保留。

发布说明见 [RELEASE_NOTES_2026.7.7.2.md](RELEASE_NOTES_2026.7.7.2.md)，完整历史更新见 [UPDATE_HISTORY.md](UPDATE_HISTORY.md)，更底层变更见 [CHANGELOG.md](CHANGELOG.md)。
