...
- Claude 可以通过本机 MCP 调 `yifanchen-zhiyi`，并完成真实 recall。
- Claude 里召回到的中文 `raw_excerpt` 不再乱码。
- Windows Claude 配置更稳：不再被 BOM 卡住，bridge 输出回到 Claude 需要的 JSON-line，普通路径和 Store 路径都能覆盖。
- OpenClaw / Hermes 记录能一起召回，同时保留来源和归属。
- computer-first 目录已经在真实记录里可见，按机器和来源工具翻记录更清楚。
- 安装提示和 Skill 会提醒 agent 在旧决定、纠错、项目边界、安装/测试/发布状态、下一步这些问题上先调记忆。
- `docs/wiki/` 已经准备好产品化 Wiki 源文件。

当前发布说明见 [RELEASE_NOTES_2026.6.2.md](RELEASE_NOTES_2026.6.2.md)，完整历史更新见 [UPDATE_HISTORY.md](UPDATE_HISTORY.md)，更底层变更见 [CHANGELOG.md](CHANGELOG.md)。
