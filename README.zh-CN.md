...
- **安装一次，自动找到本机工具**：本地页面能告诉你这台电脑上有哪些 AI 工具；支持 Skill / MCP 的会自动接入，新的记忆源按设备优先归档。
- **新工具也能更快认出来**：如果你配置了模型，忆凡尘会用它根据本机元数据识别陌生 AI 工具；如果没配置，就自动回退到本地规则。
...
- Windows 默认走原生安装；WSL 只保留给开发和高级测试。
- 官方 Windows Codex 即使 `codex.exe` 不在 PATH，也可以通过 native-host 元数据找到 bundled CLI，并用官方 `codex mcp add` 注册 `yifanchen-zhiyi`。
- Codex 召回改走当前窗口 stdio bridge，不再靠猜另一个会话。
- 默认召回是 active 分层：当前窗口/session 优先，然后同项目/同工作区、同工作流/同任务、稳定偏好/工具事实；raw-pool/global 只在明确要求更宽视图时使用。
- 持续同步状态会区分“正在跑的采集器”和“只是发现入口 / 仍待验证”的工具；Claude Desktop 本机采集默认加入循环，只写忆凡尘 raw。
- 本地对话采集器必须证明同时保存用户发言和 AI 回复，才会把工具提升为完整对话记忆来源。

当前发布说明见 [RELEASE_NOTES_2026.6.4.md](RELEASE_NOTES_2026.6.4.md)，完整历史更新见 [UPDATE_HISTORY.md](UPDATE_HISTORY.md)，更底层变更见 [CHANGELOG.md](CHANGELOG.md)。
...
- **其他本机 AI 工具**：可以从本机设置里识别；支持 Skill / MCP 的自动接入，本地格式验证后可成为记忆来源。
