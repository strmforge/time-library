...
def test_productized_loops_doctor_aggregates_five_read_only_loops(tmp_path, monkeypatch):
    ...
    assert payload["summary"]["hermes_upgrade_candidate_count"] >= 1


def test_borrowing_receipts_view_exposes_library_ids_source_refs_and_receipts(tmp_path):
    loops = importlib.import_module("src.productized_loops")

    payload = loops.build_borrowing_receipts_view_dry_run({}, memcore_root=tmp_path)
...
def test_experience_evolution_demo_reaches_apply_package_and_hermes_diff_without_writes(tmp_path):
    loops = importlib.import_module("src.productized_loops")

    payload = loops.build_experience_evolution_demo_dry_run({}, memcore_root=tmp_path)
    ...
    assert payload["validation_report"]["report_passed"] is True
    assert payload["validation_receipts"]["would_allow_apply_gate_count"] >= 1
    ...
    assert payload["flow_overview"]["ready_stage_count"] == payload["flow_overview"]["stage_count"]
    ...
    assert payload["hermes_skill_experience_diff"]["summary"]["upgrade_candidate_count"] >= 1
...
def test_productized_loops_console_routes_are_read_only(tmp_path, monkeypatch):
    ...
    assert doctor["summary"]["benchmark_best_mode"] == "zhiyi_plus_xingce"
